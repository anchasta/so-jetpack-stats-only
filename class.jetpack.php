...
class Jetpack {
	...
	private function Jetpack() {
		...
		// SAR: Removed hooks for subscribe_to_news and check_news_subscription
		 ...
		 */
		// SAR: Removed hooks for check_open_graph and check_rest_api_compat
	}
	...
	public function register_assets() {
		...
		// SAR: Removed gallery and genericons stuff
	}
	...
	*/
	...
	public static function load_modules() {
		...
		*/
		...
		// SAR: Removed call to module-extras.php
	}
	
		// SAR: Removed check_rest_api_compat function (not needed because we removed the bbpress REST API compat.)

		// SAR: Removed check_open_graph (Facebook open graph)
...
/* Jetpack Options API */
	...
	*/
	...
	public static function get_default_modules( $min_version = false, $max_version = false ) {
		...
		// SAR: Removed switch checks for inexistant modules. Leave this function to avoid removing call to this in many other parts.
	...
	public static function get_module( $module ) {
		...
		$mod['name'] = translate( $mod['name'], 'so-jetpack-stats-only' );
		$mod['description'] = translate( $mod['description'], 'so-jetpack-stats-only' );
		if ( empty( $mod['sort'] ) )
			$mod['sort'] = 10;
	...
	public static function get_active_modules() {
		...
		if ( !is_array( $active ) )
			$active = array();
		// SAR: Removed if-else for vaultpress
...
<?php
	...
	public static function plugin_activation( $network_wide ) {
		...
		if ( version_compare( $GLOBALS['wp_version'], JETPACK__MINIMUM_WP_VERSION, '<' ) ) {
			Jetpack::bail_on_activation( sprintf( __( 'Jetpack requires WordPress version %s or later.', 'so-jetpack-stats-only' ), JETPACK__MINIMUM_WP_VERSION ) );
		}
...
/**/
	...
	function throw_error_on_activate_plugin( $plugin ) {
		...
		if ( function_exists( 'stats_get_api_key' ) && in_array( 'shortlinks', $active_modules ) ) {
			...
			if ( $throw ) {
				trigger_error( sprintf( __( 'Jetpack contains the most recent version of the old &#8220;%1$s&#8221; plugin.', 'so-jetpack-stats-only' ), 'WordPress.com Stats' ), E_USER_ERROR );
			}
		}
	}
	...
	function intercept_plugin_error_scrape( $action, $result ) {
		...
		foreach ( $this->plugins_to_deactivate as $module => $deactivate_me ) {
			if ( "plugin-activation-error_{$deactivate_me[0]}" == $action ) {
				Jetpack::bail_on_activation( sprintf( __( 'Jetpack contains the most recent version of the old &#8220;%1$s&#8221; plugin.', 'so-jetpack-stats-only' ), $deactivate_me[1] ), false );
			}
		}
	}
	...
	function admin_menu() {
		...
		&&
			...
			$span_title = esc_attr( sprintf( _n( 'One New Jetpack Module', '%s New Jetpack Modules', $new_modules_count, 'so-jetpack-stats-only' ), $new_modules_count_i18n ) );
			...
			$title = __( 'SO Jetpack Stats Only', 'so-jetpack-stats-only' );
		}

		// SAR: Changed menu Jetpack to options page instead
	    $hook = add_options_page( 'SO Jetpack Stats Only', $title, 'manage_options', 'jetpack', array( $this, 'admin_page' ), '' );

    	// SAR: Workaround for Akismet menu if plugin is active
	...
	function admin_help() {
		...
		$current_screen->add_help_tab( array(
			'id'		=> 'overview',
			'title'		=> __( 'Overview', 'so-jetpack-stats-only' ),
			'content'	=> 
				'<p><strong>' . __( 'SO Jetpack Stats Only', 'so-jetpack-stats-only' ) . '</strong></p>' .
				'<p>' . __( 'The SO Jetpack Stats Only plugin is a fork of <a href="http://plugins.svn.wordpress.org/jetpack-lite/tags/2.3.4.1/">Jetpack Lite 2.3.4.1</a> and continues to have a minimal footprint without the need to install Jetpack.', 'so-jetpack-stats-only' ) . '</p>' .
				'<p>' . __( 'As the name suggests the only enabled module is the Stats module, all the rest (of Jetpack) has been disabled.', 'so-jetpack-stats-only' ) . '</p>' .
				'<p>' . __( 'If you were looking for the full <a href="http://wordpress.org/plugins/jetpack/">Jetpack version</a> or <a href="http://wordpress.org/plugins/jetpack-lite/">Jetpack Lite</a>, then click you should delete this plugin and click on the respective aforementioned links.', 'so-jetpack-stats-only' ) . '</p>'
		) );
		...
		$current_screen->set_help_sidebar(
			'<p><strong>' . __( 'For more information:', 'so-jetpack-stats-only' ) . '</strong></p>' .
			'<p><a href="http://jetpack.me/faq/" target="_blank">'     . __( 'Jetpack FAQ',     'jetpack' ) . '</a></p>' .
			'<p><a href="http://jetpack.me/support/" target="_blank">' . __( 'Jetpack Support', 'so-jetpack-stats-only' ) . '</a></p>' .
			'<p><a href="https://github.com/senlin/so-jetpack-stats-only/issues" target="_blank">' . __( 'SO Jetpack Stats Only Support', 'so-jetpack-stats-only' ) . '</a></p>'
	...
	function admin_scripts() {
		...
		wp_localize_script( 'jetpack-js', 'jetpackL10n', array(
				'ays_disconnect' => "This will deactivate the Jetpack Stats module.\nAre you sure you want to disconnect?",
				'ays_unlink'     => "This will prevent the Jetpack Stats module from working.\nAre you sure you want to unlink?",
	...
	function plugin_action_links( $actions ) {
		return array_merge(
			array( 'settings' => sprintf( '<a href="%s">%s</a>', Jetpack::admin_url(), __( 'Settings', 'so-jetpack-stats-only' ) ) ),
			$actions
		);
		return $actions;
	}
	...
	function network_connect_notice() {
		...
		<div id="message" class="updated jetpack-message">
			<div class="squeezer">
				<h4><?php _e( '<strong>Jetpack is activated!</strong> Each site on your network must be connected individually by an admin on that site.', 'so-jetpack-stats-only' ) ?></h4>
			</div>
		</div>
		<?php
	}
	...
	function admin_page_load() {
		...
		switch ( $error ) {
		case 'cheatin' :
			$this->error = __( 'Cheatin&#8217; uh?', 'so-jetpack-stats-only' );
			break;
		case 'access_denied' :
			$this->error = __( 'You need to authorize the Jetpack connection between your site and WordPress.com to enable the awesome features.', 'so-jetpack-stats-only' );
			break;
		case 'wrong_state' :
			$this->error =  __( "Don&#8217;t cross the streams!  You need to stay logged in to your WordPress blog while you authorize Jetpack.", 'so-jetpack-stats-only' );
			break;
		case 'invalid_client' :
			// @todo re-register instead of deactivate/reactivate
			$this->error = __( 'Return to sender.  Whoops! It looks like you got the wrong Jetpack in the mail; deactivate then reactivate the Jetpack plugin to get a new one.', 'so-jetpack-stats-only' );
			break;
		case 'invalid_grant' :
			$this->error = __( "Wrong size.  Hm&#8230; it seems your Jetpack doesn&#8217;t quite fit.  Have you lost weight? Click &#8220;Connect to WordPress.com&#8221; again to get your Jetpack adjusted.", 'so-jetpack-stats-only' );
			break;
		case 'site_inaccessible' :
		case 'site_requires_authorization' :
			$this->error = sprintf( __( 'Your website needs to be publicly accessible to use Jetpack: %s', 'so-jetpack-stats-only' ), "<code>$error</code>" );
			break;
		case 'module_activation_failed' :
			$module = Jetpack::state( 'module' );
			if ( !empty( $module ) && $mod = Jetpack::get_module( $module ) ) {
				$this->error = sprintf( __( '%s could not be activated because it triggered a <strong>fatal error</strong>. Perhaps there is a conflict with another plugin you have installed?', 'so-jetpack-stats-only' ), $mod['name'] );
				if ( isset( $this->plugins_to_deactivate[$module] ) ) {
					$this->error .= ' ' . sprintf( __( 'Do you still have the %s plugin installed?', 'so-jetpack-stats-only' ), $this->plugins_to_deactivate[$module][1] );
				}
			} else {
				$this->error  = __( 'Module could not be activated because it triggered a <strong>fatal error</strong>. Perhaps there is a conflict with another plugin you have installed?', 'so-jetpack-stats-only' );
			}
			...
			break;
		case 'not_public' :
			$this->error = __( "<strong>Your Jetpack has a glitch.</strong> Connecting this site with WordPress.com is not possible. This usually means your site is not publicly accessible (localhost).", 'so-jetpack-stats-only' );
			break;
		case 'wpcom_408' :
		case 'wpcom_5??' :
		case 'wpcom_bad_response' :
		case 'wpcom_outage' :
			$this->error = __( 'WordPress.com is currently having problems and is unable to fuel up your Jetpack.  Please try again later.', 'so-jetpack-stats-only' );
			break;
		case 'register_http_request_failed' :
		case 'token_http_request_failed' :
			$this->error = sprintf( __( 'Jetpack could not contact WordPress.com: %s.  This usually means something is incorrectly configured on your web host.', 'so-jetpack-stats-only' ), "<code>$error</code>" );
			break;
		default :
			if ( empty( $error ) ) {
				break;
		...
		case 'verify_secrets_mismatch' :
			...
			$this->error = sprintf( __( "<strong>Your Jetpack has a glitch.</strong>  Something went wrong that&#8217;s never supposed to happen.  Guess you&#8217;re just lucky: %s", 'so-jetpack-stats-only' ), "<code>$error</code>" );
			if ( !Jetpack::is_active() ) {
				$this->error .= '<br />';
				$this->error .= sprintf( __( 'Try connecting again.', 'so-jetpack-stats-only' ) );
			}
			break;
		}
		...
		case 'modules_activated' :
			$this->message = sprintf(
				__( 'Welcome to <strong>Jetpack %s</strong>!', 'so-jetpack-stats-only' ),
				JETPACK__VERSION
			);
			...
			if ( $active_state ) {
				...
				if ( $titles ) {
					$this->message .= '<br /><br />' . wp_sprintf( __( 'The following new modules have been activated: %l.', 'so-jetpack-stats-only' ), $titles );
				}
			}
			...
			if ( $reactive_state = Jetpack::state( 'reactivated_modules' ) ) {
				...
				if ( $titles ) {
					$this->message .= '<br /><br />' . wp_sprintf( __( 'The following modules have been updated: %l.', 'so-jetpack-stats-only' ), $titles );
				}
			}

			// SAR: removed call to jetpack_comment_notice
			break;
		...
		case 'module_configured' :
			$this->message = __( '<strong>Module settings were saved.</strong> ', 'so-jetpack-stats-only' );
			break;

		case 'already_authorized' :
			$this->message = __( '<strong>Your Jetpack is already connected.</strong> ', 'so-jetpack-stats-only' );
			break;

		case 'authorized' :
			$this->message  = __( "<strong>The Jetpack Stats module has been activated.</strong> ", 'so-jetpack-stats-only' );
			$this->message .= "<br />\n";
			$this->message .= __( 'Click on the Configure button to go to the settings.', 'so-jetpack-stats-only' );
			break;
		...
		case 'unlinked' :
			...
			$this->message = sprintf( __( '<strong>You have unlinked your account (%s) from WordPress.com.</strong>', 'so-jetpack-stats-only' ), $user->user_login );
			break;
		}
...
<div id="message" class="jetpack-message jetpack-err">
	<div class="squeezer">
		<h4><strong><?php esc_html_e( 'Is this site private?', 'so-jetpack-stats-only' ); ?></strong></h4><br />
		<p><?php
			...
			echo wp_kses( sprintf(
				...
				esc_attr( wp_kses( wp_sprintf( _x( 'Deactivate %l', '%l = list of Jetpack module/feature names', 'so-jetpack-stats-only' ), $module_names ), array() ) )
...
<?php
	...
	?>
		<div class="wrap" id="jetpack-settings">
				...
				<div id="jp-clouds">
					...
					<div id="jp-disconnectors">
						...
						<div id="jp-disconnect" class="jp-disconnect">
							<a href="<?php echo wp_nonce_url( Jetpack::admin_url( 'action=disconnect' ), 'jetpack-disconnect' ); ?>"><div class="deftext"><?php _e( 'Connected to WordPress.com', 'so-jetpack-stats-only' ); ?></div><div class="hovertext"><?php _e( 'Disconnect from WordPress.com', 'so-jetpack-stats-only' ) ?></div></a>
						</div>
						<?php endif; ?>
						<?php if ( $is_user_connected && !$is_master_user ) : ?>
						<div id="jp-unlink" class="jp-disconnect">
							<a href="<?php echo wp_nonce_url( Jetpack::admin_url( 'action=unlink' ), 'jetpack-unlink' ); ?>"><div class="deftext"><?php _e( 'User linked to WordPress.com', 'so-jetpack-stats-only' ); ?></div><div class="hovertext"><?php _e( 'Unlink user from WordPress.com', 'so-jetpack-stats-only' ) ?></div></a>
						</div>
						<?php endif; ?>
					</div>
					<?php endif; ?>
					<h3><?php _e( 'Jetpack by WordPress.com', 'so-jetpack-stats-only' ) ?></h3>
				</div>
			</div>

			<?php if ( isset( $_GET['jetpack-notice'] ) && 'dismiss' == $_GET['jetpack-notice'] ) : ?>
				<div id="message" class="error">
					<p><?php _e( 'Jetpack is network activated and notices can not be dismissed.', 'so-jetpack-stats-only' ); ?></p>
				</div>
			<?php endif; ?>
			...
			?>
				...
				<div id="message" class="updated jetpack-message jp-connect">
					<div id="jp-dismiss" class="jetpack-close-button-container">
						<a class="jetpack-close-button" href="?page=jetpack&jetpack-notice=dismiss" title="<?php _e( 'Dismiss this notice.', 'so-jetpack-stats-only' ); ?>"><?php _e( 'Dismiss this notice.', 'so-jetpack-stats-only' ); ?></a>
					</div>
					<div class="jetpack-wrap-container">
						<div class="jetpack-text-container">
							<h4>
								<p><?php _e( "To enable the Jetpack Stats module you&#8217;ll need to connect your website to WordPress.com using the button to the right. Once you&#8217;ve made the connection you can activate the Stats module below.", 'so-jetpack-stats-only' ) ?></p>
							</h4>
						</div>
						<div class="jetpack-install-container">
							<p class="submit"><a href="<?php echo $this->build_connect_url() ?>" class="button-connector" id="wpcom-connect"><?php _e( 'Connect to WordPress.com', 'so-jetpack-stats-only' ); ?></a></p>
						</div>
					</div>
				</div>
				...
				<div id="message" class="updated jetpack-message jp-connect">
					<div class="jetpack-wrap-container">
						<div class="jetpack-text-container">
							<h4>
								<p><?php _e( "To enable the Jetpack Stats module you&#8217;ll need to link your account here to your WordPress.com account using the button to the right.", 'so-jetpack-stats-only' ) ?></p>
							</h4>
						</div>
						<div class="jetpack-install-container">
							<p class="submit"><a href="<?php echo $this->build_connect_url() ?>" class="button-connector" id="wpcom-connect"><?php _e( 'Link account with WordPress.com', 'so-jetpack-stats-only' ); ?></a></p>
						</div>
					</div>
				</div>
			...
			printf( '<p>' . __( '<em>SO Jetpack Stats Only</em> is a fork of <em><a href="%1$s">Jetpack Lite 2.3.4.1</a></em> (by <a href="%2$s">Samuel Aguilera</a>) which in turn is a fork of Jetpack.', 'so-jetpack-stats-only' ) . '</p>' ,
				'http://plugins.svn.wordpress.org/jetpack-lite/tags/2.3.4.1/',
				'http://profiles.wordpress.org/samuelaguilera/'
			);
			echo '<p>' . __( 'Neither fork adds any functionality to Jetpack itself, they merely remove the bloat that ships with Jetpack by default.', 'so-jetpack-stats-only' ) . '</p>';
			...
			<p class="rate-this-plugin">
				...
				printf( __( 'If you have found this plugin at all useful, please give it a favourable rating in the <a href="%s" title="Rate this plugin!">WordPress Plugin Repository</a>.', 'so-jetpack-stats-only' ), 
					esc_url( 'http://wordpress.org/plugins/so-jetpack-stats-only/' )
				);
				?>
			...
			<div class="author postbox">
				...
				<h3 class="hndle">
					<span><?php _e( 'About the Author', 'so-jetpack-stats-only' ); ?></span>
				</h3>
				
				<div class="inside">
					<img class="author-image" src="http://www.gravatar.com/avatar/<?php echo md5( 'info@senlinonline.com' ); ?>" />
					<p>
						<?php printf( __( 'Hi, my name is Piet Bos, I hope you like this plugin! Please check out any of my other plugins on <a href="%s" title="SO WP Plugins">SO WP Plugins</a>. You can find out more information about me via the following links:', 'so-jetpack-stats-only' ),
						esc_url( 'http://so-wp.com/' )
						); ?>
					</p>
					
					<ul>
						<li><a href="http://senlinonline.com/" target="_blank" title="Senlin Online"><?php _e('Senlin Online', 'jetpack'); ?></a></li>
						<li><a href="http://wpti.ps/" target="_blank" title="WP TIPS"><?php _e('WP Tips', 'jetpack'); ?></a></li>
						<li><a href="https://plus.google.com/+PietBos" target="_blank" title="Piet on Google+"><?php _e( 'Google+', 'so-jetpack-stats-only' ); ?></a></li>
						<li><a href="http://cn.linkedin.com/in/pietbos" target="_blank" title="LinkedIn profile"><?php _e( 'LinkedIn', 'so-jetpack-stats-only' ); ?></a></li>
						<li><a href="http://twitter.com/piethfbos" target="_blank" title="Twitter"><?php _e( 'Twitter: @piethfbos', 'so-jetpack-stats-only' ); ?></a></li>
						<li><a href="http://github.com/senlin" title="on Github"><?php _e( 'Github', 'so-jetpack-stats-only' ); ?></a></li>
						<li><a href="http://profiles.wordpress.org/senlin/" title="on WordPress.org"><?php _e( 'WordPress.org Profile', 'so-jetpack-stats-only' ); ?></a></li>
					</ul>
	...
	function admin_screen_list_modules() {
		...
		foreach ( (array) $available as $module_data ) {
			...
			if ( $activated ) {
				$css        = 'active';
				$toggle     = __( 'Deactivate', 'so-jetpack-stats-only' );
				...
				$css        = 'inactive';
				$toggle     = __( 'Activate', 'so-jetpack-stats-only' );
				$toggle_url = wp_nonce_url(
					Jetpack::admin_url( array(
						'page'   => 'jetpack',
						'action' => 'activate',
			...
			$free_text = esc_html( $module_data['free'] ?  __( 'Free', 'so-jetpack-stats-only' ) : __( 'Purchase', 'so-jetpack-stats-only' ) );
			...
			<div class="jetpack-module jetpack-<?php echo $classes; ?>" id="<?php echo $module ?>">
				...
				<?php if ( $jetpack_connected || ( Jetpack::is_development_mode() && ! $module_data['requires_connection'] ) ) : ?>
					...
					if ( current_user_can( 'manage_options' ) && apply_filters( 'jetpack_module_configurable_' . $module, false ) ) {
						echo '<a href="' . esc_url( Jetpack::module_configuration_url( $module ) ) . '" class="jetpack-configure-button button-secondary">' . __( 'Configure', 'so-jetpack-stats-only' ) . '</a>';
					}
	...
	// SAR: Removed check_news_subscription

	// SAR: Removed subscribe_to_news function
...
/* Client API */
	...
	public function alert_required_ssl_fail() {
		...
		<div id="message" class="error jetpack-message jp-identity-crisis">
			<div class="jetpack-wrap-container">
				<div class="jetpack-text-container">
					<h3><?php _e( 'Something is being cranky!', 'so-jetpack-stats-only' ); ?></h3>
					<p><?php _e( 'Your site is configured to only permit SSL connections to Jetpack, but SSL connections don\'t seem to be functional!', 'so-jetpack-stats-only' ); ?></p>
				</div>
			</div>
		</div>
	...
	public static function register() {
		...
		if ( 5 == $code_type ) {
			return new Jetpack_Error( 'wpcom_5??', sprintf( __( 'Error Details: %s', 'so-jetpack-stats-only' ), $code ), $code );
		} elseif ( 408 == $code ) {
			return new Jetpack_Error( 'wpcom_408', sprintf( __( 'Error Details: %s', 'so-jetpack-stats-only' ), $code ), $code );
			...
			return new Jetpack_Error( 'wpcom_bad_response', sprintf( __( 'Error Details: %s', 'so-jetpack-stats-only' ), $code ), $code );
		}

		// Jetpack ID error block
		if ( empty( $json->jetpack_id ) ) {
			return new Jetpack_Error( 'jetpack_id', sprintf( __( 'Error Details: Jetpack ID is empty. Do not publicly post this error message! %s', 'so-jetpack-stats-only' ), $entity ), $entity );
		} elseif ( ! is_scalar( $json->jetpack_id ) ) {
			return new Jetpack_Error( 'jetpack_id', sprintf( __( 'Error Details: Jetpack ID is not a scalar. Do not publicly post this error message! %s', 'so-jetpack-stats-only' ) , $entity ), $entity );
		} elseif ( preg_match( '/[^0-9]/', $json->jetpack_id ) ) {
			return new Jetpack_Error( 'jetpack_id', sprintf( __( 'Error Details: Jetpack ID begins with a numeral. Do not publicly post this error message! %s', 'so-jetpack-stats-only' ) , $entity ), $entity);
		}
...
// SAR: Removed functions for JSON API authorization (we removed that module, so we dont need these functions)
...
*/
	...
	public function alert_identity_crisis() {
		...
		<div id="message" class="updated jetpack-message jp-identity-crisis">
			<div class="jetpack-wrap-container">
				<div class="jetpack-text-container">
					<h3><?php _e( 'Something has gotten mixed up!', 'so-jetpack-stats-only' ); ?></h3>
					<?php foreach ( $errors as $key => $value ) : ?>
						<p><?php printf( __( 'Your <code>%1$s</code> option is set up as <strong>%2$s</strong>, but your WordPress.com connection lists it as <strong>%3$s</strong>!', 'so-jetpack-stats-only' ), $key, (string) get_option( $key ), $value ); ?></p>
					<?php endforeach; ?>
					<p><?php _e( 'This problem can often be resolved by disconnecting, then reconnecting to WordPress.com.', 'so-jetpack-stats-only' ); ?> <a href="<?php echo $this->build_reconnect_url() ?>" class="button-connector" id="wpcom-connect"><?php _e( 'Disconnect and Reconnect to WordPress.com', 'so-jetpack-stats-only' ); ?></a></p>
				</div>
			</div>
		</div>
